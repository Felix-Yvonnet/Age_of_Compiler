...
object DrawInitial:
  ...
  def buildWallArroundCentrale(scene: GameMap) =
    for k <- 1 to 5 if k != 3 do scene.placeSthg(Wall(Point(scene.width - k, scene.height - 6)))
    for k <- 1 to 5 if k != 3 do scene.placeSthg(Wall(Point(scene.width - 6, scene.height - k)))

    for k <- 1 to 7 if k != 5 do scene.placeSthg(Wall(Point(scene.width - k, scene.height - 8)))
    for k <- 1 to 7 if k != 5 do scene.placeSthg(Wall(Point(scene.width - 8, scene.height - k)))
